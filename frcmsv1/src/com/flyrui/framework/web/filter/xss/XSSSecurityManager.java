...
public class XSSSecurityManager {
    ...
    public static void init(FilterConfig config){
    	...
    	//初始化过滤配置文件
        String xssPath = "/com/flyrui/framework/web/filter/xss/xss_security_config.xml";
	...
	public static boolean initConfig(String path) throws DocumentException {
		...
		Element superElement = new SAXReader().read(XSSSecurityManager.class.getResourceAsStream(path)).getRootElement();
		XSSSecurityConfig.IS_CHECK_HEADER = new Boolean(getEleValue(superElement,XSSSecurityCon.IS_CHECK_HEADER)).booleanValue();
		XSSSecurityConfig.IS_CHECK_PARAMETER = new Boolean(getEleValue(superElement,XSSSecurityCon.IS_CHECK_PARAMETER)).booleanValue();
		XSSSecurityConfig.IS_LOG = new Boolean(getEleValue(superElement,XSSSecurityCon.IS_LOG)).booleanValue();
		XSSSecurityConfig.IS_CHAIN = new Boolean(getEleValue(superElement,XSSSecurityCon.IS_CHAIN)).booleanValue();
		XSSSecurityConfig.REPLACE = new Boolean(getEleValue(superElement,XSSSecurityCon.REPLACE)).booleanValue();
