...
import com.flyrui.common.excel.ExcelStyle;
...
public class ExcelExport<T> {
	...
	public ExcelExport(String templateId){
	...
	public void exportExcel(String title, Collection<T> dataset,OutputStream out) {
		...
		try {
			...
			for (int i = 0; i < filed.length; i++) {
				...
				if(busTemplateItem!=null&&"1".equals(busTemplateItem.getIs_export())){
					
					
					// 添加到标题
					exportfieldtile.add(busTemplateItem.getItem_name());
					// 添加到需要导出的字段的方法
