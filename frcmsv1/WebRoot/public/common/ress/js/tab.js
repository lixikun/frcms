...
function TabView(option) {
	...
	function initTab(el,page) {
		...
		if (tab_context.current == null || tab_context.current != this) {
			$(theTab).mouseover( function() {
				...
				tab_context.current_ops = page.ids[$(this).attr("id")];
	...
	this.init = function() {
		...
		$("#icon_frash").click(function(){
			var ops = tab_context.current_ops;
			if(ops){
				tab.update(ops);
				//close(ops.id);
			    //tab.add( ops);
	...
	this.add = function(option) {
	   if(this.ids.page_size>=8){
			alert("打开栏目超过8个，请关闭部分再打开！");
			return;
		}
		var id = option.id;
		if(this.ids[id]){
		...
		initTab($("#" + op1.id),this);
		//initTab2(op1.id,option.title,option.url);//fresh
		if (!op1.isClosed) {
