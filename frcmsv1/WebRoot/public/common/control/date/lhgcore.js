/*
 * @lhgcore - JavaScript Library v1.0.4 - Date : 2009-9-23 @Copyright lhgcore.js
 * (c) 2009 By LiHuiGang Reserved
 */
(function() {
		...
		apch : function(c, y) {
			switch (y) {
				...
				case 'be' :
					...
					break;
				case 'af' :
					return this[0].parentNode.insertBefore(c,
							this[0].nextSibling);
					break;
				default :
	...
	g.ret.exend({
				stopac : function(o) {
					if (g.ie) {
						o = Math.round(o * 100);
						this[0].style.filter = (o > 100 ? '' : 'alpha(opacity='
								+ o + ')')
					} else
	...
	J.exend(J.panel.prototype, {
		...
		show : function(x, y, e, w, h) {
			...
			var b = e.nodeType == 9
					? J.idtd(e) ? e.documentElement : e.body
					: e;
			var c = J.dpos(this._win, b);
			x += c.x;
			y += c.y;
			var d = J.vsiz(this._win), sp = J.spos(this._win), vh = d.h + sp.y, vw = d.w
					+ sp.x;
			if ((x + iw) > vw)
				x -= x + iw - vw;
			if ((y + ih) > vh)
