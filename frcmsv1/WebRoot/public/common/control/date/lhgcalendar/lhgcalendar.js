/*
 *@Generator -> Calendar Plugins V2.0.2 - Date : 2009-9-19
 *@Copyright lhgcore.js (c) 2009 By LiHuiGang Reserved
 */

var calendar = function(b)
{
    var p = this._p = new J.panel(b), d = new Date();
    p.applnk('/public/common/control/date/lhgcalendar/lhgcalendar.css' ); J(p._main).acls('cal_panel');

    if(J.clean) lhgcore.clean.items( this, this.c_clean );
    this.yy = d.getFullYear(); this.mm = d.getMonth() + 1; this.dd = d.getDate();
...
J.exend( calendar.prototype, {
    ...
    createbody: function(d,m)
    {
        ...
        txt = '<table width="100%" cellspacing="0" cellpadding="0" border="0">' +
            ...
            '<div id="foot"><span id="t1">今天</span><span id="t3">清空</span><span id="t2">' + this.yy + '-' + this.mm + '-' + this.dd + '</span></div>';
        ...
        }).click(function(){ J.calendar.setdate('t'); });
                    ...
                    if(day)
                    {
                        if( this.than && this.than == 'min' )
                        {
                            if( this.y < this.ty || (this.y == this.ty && this.m < this.tm) || (this.y == this.ty && this.m == this.tm && day < this.td)
                                || (this.y == this.ty && this.m == this.tm && day == this.td) )
                            {
    ...
    to : function(l)
    {
        var a = l.split(','), v = J('#'+a[0]) ? J.trim(J(a[0]).val()) : a[0]; this.than = a[1];
        if( J('#'+a[0]) && J(a[0]).isnl() )
        {
        ...
        if( v == 'today' )
        {
            this.ty = this.yy; this.tm = this.mm; this.td = this.dd;
        }
        else
        {
    ...
    setdate : function(t)
    {
        ...
        if( t == 't' ){ c.y = c.yy; c.m = c.mm; d = c.dd; }
    ...
    over : function()
    {
        ...
        J('t2',c._p._doc).html( c.y + '-' + c.m + '-' + d );
    },

    out : function()
    {
        if( this.className != 'tday' ) J(this).acls('tdday'); var c = J.calendar;
        J('t2',c._p._doc).html( c.yy + '-' + c.mm + '-' + c.dd );
    },
